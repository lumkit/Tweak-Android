...
class AppListHelper(context: Context, private val getTags: Boolean = true) {
    ...
    private fun exclude(packageName: String): Boolean = packageName.contains(".overlay") ||
    ...
    private fun getApplicationInfo(
        ...
        item.path = appPath
        item.updated =
            isSystemApp(applicationInfo) && (appPath.startsWith("/data") || (applicationInfo.flags and ApplicationInfo.FLAG_EXTERNAL_STORAGE) != 0)
    ...
    fun getBootableApps(
        ...
        for (i in packageInfos.indices) {
            ...
            if ((systemApp == false && isSystemApp(applicationInfo)) || (systemApp == true && !isSystemApp(
                    applicationInfo
                ))
            )
                continue
            ...
            // item.appType = if (applicationInfo.sourceDir.startsWith("/system")) AppType.SYSTEM else AppType.USER
            item.appType =
                if ((applicationInfo.flags and ApplicationInfo.FLAG_SYSTEM) == 0) AppType.USER else AppType.SYSTEM
            try {
                val packageInfo = packageManager.getPackageInfo(applicationInfo.packageName, 0)
                item.versionName = packageInfo.versionName ?: ""
                item.versionCode = packageInfo.versionCode
        ...
        for (i in files.indices) {
            ...
            try {
                ...
                if (packageInfo != null) {
                    ...
                    item.versionName = packageInfo.versionName ?: ""
                    item.versionCode = packageInfo.versionCode
                    item.appType = AppType.BACKUP_FILE
                    list.add(item)
                }
            } catch (_: Exception) {
            }
